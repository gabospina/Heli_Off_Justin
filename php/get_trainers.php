<?php
	...
	WHERE p.id NOT IN (SELECT pilot_id FROM training_schedule WHERE `date`>='$backSeven' AND `date` <='$end') AND p.id NOT IN (SELECT id FROM available WHERE `on` <= '$end' AND `off`>='$start') AND p.account=$account ORDER BY p.lname";
